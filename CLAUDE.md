...
- **`templates/original_export.docx` is read-only except via the documented re-export
  ...
  against. Do not hand-edit it. Replace it only by copying a fresh Google Docs export over
  it (CLI) or uploading through the Template tab, then regenerating
  `main_template.docx` with `scripts/build_template.py`.
- **`templates/main_template.docx` is generated by exactly one script.** Only
  ...
  update. The resume genuinely changes (it was replaced once mid-development). The web
  Template tab shells out to that same script; it does not tag the document itself.
...
descriptions), and `--no-facets` (skip LLM selection of project tech / coursework;
pools are still truncated to the one-line / two-line budgets in listed order).
...
job description ────┘         │           canonicalised against the resume's own tags
                    ...
                    facets.select_facets() project tech + coursework    (LLM, once, cached)
                              │            code enforces 1-line / 2-line budgets;
                              │            --no-facets still truncates pools in listed order
                              ▼
...
- **Education coursework is tailored; skill groups are not.** `facets.select_facets`
  picks which coursework titles to show (from the master pool, original names only) and
  which project tech labels to show (≤4, with optional JD-anchored renames). Skill groups
  still render in full every run. Coursework and skills both count toward
  `fit._fixed_overhead_lines`, but a trimmed coursework line frees budget the fit loop can
  reclaim. Only experience/project *bullets* are varied inside the fit loop.
...
stop, and the fonts. The name line, contact, and EDUCATION are all tagged so the display
name, LinkedIn/GitHub, coursework, and GPA come from `master_resume.json` without a
re-export.
